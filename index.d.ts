...
export interface BuildConfig {
  ...
  /**
   * Controls generation of the per-resource `.opencore/opencore.gen.ts` files that give
   * end-to-end type safety to net events, RPCs and commands.
   *
   * @example
   * ```ts
   * build: {
 ...
 *
 * @remarks
 * The CLI scans each resource for `@Server.OnNet`, `@Client.OnNet`, `@Server.OnRPC`,
 * `@Client.OnRPC` and `@Server.Command` decorators and emits a `.opencore/opencore.gen.ts`
 * that augments the framework's `Register` interface. Emitting sites — `player.emit()`,
 * `events.emit()`, `rpc.call()`, `webView.send()` — then autocomplete to the declared event
 * names and check their payloads against the handler signatures.
 *
 * Generated files live under `.opencore/`, which the starter `.gitignore` already excludes,
 * and are regenerated by both `opencore dev` and `opencore build`.
 */
export interface TypegenConfig {
  /**
   * Enable type generation.
   *
   * When disabled, existing generated files are deleted and every signature reverts to its
   * previous loose form — no source changes required.
   *
   * @default true
   */
  enabled?: boolean;
  ...
  /**
   * Treat unknown event names as compile errors.
   *
   * Off by default because typegen is scoped per resource: a resource legitimately emits
   * events handled by another resource or by the framework's own `SYSTEM_EVENTS`, and those
   * names are absent from its own generated map. With `strict: false` those names are still
   * accepted while known names keep their autocomplete and payload checking.
   *
   * @default false
   */
  strict?: boolean;
